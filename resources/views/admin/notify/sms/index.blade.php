                        ...
                        <tbody>
                            @foreach ($sms as $key => $single_sms)
                                <tr>
                                    ...
                                    <td>
                                        <label for="">
                                            <input type="checkbox" data-url="{{route('admin.notify.sms.status', $single_sms->id)}}" id="{{$single_sms->id}}" onchange="changeStatus({{$single_sms->id}})" @if ($single_sms->status === 1)
                                                checked
                                            @endif>
                                        </label>
                                    </td>
                                    <td class="text-start width-16-rem">
                                        <a href="{{route('admin.notify.sms.edit', $single_sms->id)}}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i> ویرایش</a>
                                        <form action="{{route('admin.notify.sms.destroy', $single_sms->id)}}" class="d-inline" method="post">
                                            @csrf
                                            @method('DELETE')
