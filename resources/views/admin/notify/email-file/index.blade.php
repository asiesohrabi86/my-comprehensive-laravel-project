                        ...
                        <tbody>
                            @foreach ($email->files as $key => $file)
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" id="{{$file->id}}" data-url="{{route('admin.notify.email-file.status', $file->id)}}" onchange="changeStatus({{$file->id}})" @if ($file->status == 1)
                                            checked
                                        @endif>
                                    </td>
                                    <td class="text-start width-16-rem">
                                        <a href="{{route('admin.notify.email-file.edit', $file->id)}}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i> ویرایش</a>
                                        <form action="{{route('admin.notify.email-file.destroy', $file->id)}}" class="d-inline" method="post">
                                            @csrf
                                            @method('DELETE')
