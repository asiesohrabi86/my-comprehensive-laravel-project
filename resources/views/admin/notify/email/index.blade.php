                        ...
                        <tbody>
                            @foreach ($emails as $key => $email)
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" id="{{$email->id}}" data-url="{{route('admin.notify.email.status', $email->id)}}" onchange="changeStatus({{$email->id}})" @if ($email->status == 1)
                                            checked
                                        @endif>
                                    </td>
                                    <td class="text-start width-16-rem">
                                        <a href="{{route('admin.notify.email-file.index', $email->id)}}" class="btn btn-sm btn-warning"><i class="fa fa-file"></i> فایل های ضمیمه شده</a>
                                        <a href="{{route('admin.notify.email.edit', $email->id)}}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i> ویرایش</a>
                                        <form action="{{route('admin.notify.email.destroy', $email->id)}}" class="d-inline" method="post">
                                            @csrf
                                            @method('DELETE')
