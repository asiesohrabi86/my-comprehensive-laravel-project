                        ...
                        <tbody>
                            @foreach($products as $product)
                                <tr>
                                    ...
                                    <td>{{$product->name}}</td>
                                    <td><img src="{{asset($product->image['indexArray'][$product->image['currentImage']])}}" alt="" width="100" height="50" class="max-height-2rem"></td>
                                    <td>{{$product->marketable_number}}</td>
                                    <td>{{$product->frozen_number}}</td>
                                    <td>{{$product->sold_number}}</td>
                                    <td class="text-start width-22-rem">
                                        <a href="{{route('admin.market.store.add-to-store', $product->id)}}" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i> افزایش موجودی</a>
