                        ...
                        <tbody>
                            @foreach ($comments as $key => $comment) 
                                <tr>
                                    ...
                                    <td>
                                        <label for="">
                                            <input type="checkbox" id="{{$comment->id}}" data-url = "{{route('admin.market.comment.status', $comment->id)}}"
                                            onchange="changeStatus({{$comment->id}})" @if ($comment->status == 1) checked @endif>
                                        </label>
                                    </td>
                                    <td class="text-start width-16-rem">
                                        <a href="{{route('admin.market.comment.show', $comment->id)}}" class="btn btn-sm btn-info"><i class="fa fa-eye"></i> نمایش</a>
                                        {{-- <a href="{{route('admin.market.comment.approved', $comment->id)}}" class="btn {{$comment->approved == 1 ? 'btn-warning' : 'btn-success'}} btn-sm"><i class="fa {{$comment->approved == 1 ? 'fa-clock' : 'fa-check'}}"></i> {{$comment->approved == 1 ? 'عدم تایید' : 'تایید'}}
                                        </a> --}}
                                        @if ($comment->approved == 1)
                                            <a href="{{route('admin.market.comment.approved', $comment->id)}}" class="btn btn-warning btn-sm"><i class="fa fa-clock text-white"></i> عدم تایید</a>
                                        @else
