                        ...
                        <section class="row">
                            <section class="col-12 col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control form-control-sm" type="text" name="name" value="{{old('name', $productCategory->name)}}">
                                </div>
                                @error('name')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-control-sm" name="parent_id">
                                        ...
                                        @foreach ($parent_categories as $parent_category)
                                            <option value="{{$parent_category->id}}" @if (old('parent_id', $productCategory->parent_id) == $parent_category->id)
                                            selected  
                                            @endif>{{$parent_category->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @error('name')
                            ...
                            <section class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control form-control-sm @error('description') is-invalid @enderror" rows="6" name="description" id="description">{{old('description', $productCategory->description)}}</textarea>
                                </div>
                                @error('description')
                                    <span class="alert_required bg-danger text-white p-1 rounded" role="alert">
                                        <strong>{{$message}}</strong>
                                    ...
                                    @foreach ($productCategory->image['indexArray'] as $key => $value)
                                        <section class="col-md-{{ 6/ $number}}">
                                            <div class="form-check">
                                                <input type="radio" name="current-image" class="form-ckeck-input" value="{{$key}}" id="{{$number}}" @if ($productCategory->image['currentImage'] == $key)
                                                 checked 
                                                @endif
                                                >
                            ...
                            <section class="col-12 col-md-6 my-2">
                                <div class="form-group">
                                    ...
                                    <input class="form-control form-control-sm @error('tags') is-invalid @enderror" type="hidden" name="tags" id="tags" value="{{old('tags', $productCategory->tags)}}">
