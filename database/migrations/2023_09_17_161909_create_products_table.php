...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('products', function (Blueprint $table) {
            ...
            $table->decimal('length', 10, 1)->comment('cm nuit');
            $table->decimal('width', 10, 1)->comment('cm nuit');
            $table->decimal('height', 10, 1)->comment('cm nuit');
            $table->decimal('price', 20, 3);
            $table->tinyInteger('status')->default(0);
            $table->tinyInteger('marketable')->default(1)->comment('1 => marketable, 1 => is not marketable');
            $table->string('tags');
